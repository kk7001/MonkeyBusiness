...
@router.post("/{gameinfo}/IIDX31pc/oldget")
async def iidx31pc_oldget(request: Request):
    request_info = await core_process_request(request)
    root_attrib = request_info["root"][0].attrib
    cid = root_attrib.get("cid", root_attrib.get("refid", ""))
    if not cid:
        response = E.response(E.IIDX31pc(status=1))
        response_body, response_headers = await core_prepare_response(request, response)
        return Response(content=response_body, headers=response_headers)
        ...
        response = E.response(
            E.IIDX31pc(
                E.pcdata(
                    ...
                    pid=old_profile.get("region", 1),
                    mode=old_profile.get("mode", 0),
                    d_hispeed=old_profile.get("d_hispeed", 0),
                    s_hispeed=old_profile.get("s_hispeed", 0),
                ),
                E.qprodata(
                    [old_profile.get("head", 0), old_profile.get("hair", 0),
                     old_profile.get("face", 0), old_profile.get("hand", 0),
                     old_profile.get("body", 0)],
                    __type="u32",
                ),
            )
        )
